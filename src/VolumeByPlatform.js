  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (cachedData && cachedTimestamp && currentTime - parseInt(cachedTimestamp) < CACHE_EXPIRATION) {
        ...
        try {
          const { data: { result: { rows } } } = await axios.get('https://api.dune.com/api/v1/query/2647026/results?api_key=piyMny0VjuHCs1Mk5KLTXCwOpclpXcvQ');

          const transformedData = rows.reduce((acc, { time, platform, 'volume': volume }) => {
...
export default VolumeByPlatform;
